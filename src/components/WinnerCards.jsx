  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ borderRadius: 24, overflow: 'hidden', boxShadow: '0 16px 48px rgba(0,0,0,0.22)', marginBottom: 10 }}>
        <div style={{ background: 'linear-gradient(160deg, #1a2a46 0%, #111f35 55%, #0b1527 100%)', padding: '28px 20px 24px', textAlign: 'center', position: 'relative' }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(148,163,184,0.75)', textTransform: 'uppercase', letterSpacing: '3px', marginBottom: 14 }}>{tournament?.name}</div>
          <div style={{ fontSize: 72, lineHeight: 1, marginBottom: 16, filter: 'drop-shadow(0 0 18px rgba(90,110,156,0.7)) drop-shadow(0 4px 8px rgba(0,0,0,0.4))' }}>🏆</div>
          {tournament?.winnerPhoto && (
            <div style={{ display: 'inline-block', marginBottom: 16 }}>
              <img src={tournament.winnerPhoto} alt="champion" style={{ width: 160, height: 160, objectFit: 'cover', borderRadius: '50%', border: '5px solid #5a6e9c', boxShadow: '0 0 0 8px rgba(90,110,156,0.2), 0 8px 32px rgba(0,0,0,0.4)' }} />
            </div>
          )}
